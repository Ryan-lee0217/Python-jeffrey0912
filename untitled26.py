# -*- coding: utf-8 -*-
"""
Created on Sat Dec  5 15:49:57 2020
...
class Grid:
    ...
    def retrieve_empty_cells(self):
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.cells[i][j] == 0:#如果該格子等於0,則在empty_cells新增
            ...
            for j in range(self.size):
                ...
                #fg->foreground(前景)
                    ...
                    if self.grid.cells[i][j] > 2048:
                        """color等於預設值"""
                        bg_color = GamePanel.CELL_BACKGROUND_COLOR_DICT.get('beyond')#數字無法再移動
...
class Game:
    ...
    def you_win(self):
        self.won = True
        print('You Win!')
        messagebox.showinfo(title="你贏了",message="恭喜，你贏了")

    """遊戲失敗"""
    def game_over(self):
        print('Game over!')
        messagebox.showinfo(title="你輸了",message=": (")
